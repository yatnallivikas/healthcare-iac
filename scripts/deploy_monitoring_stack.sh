...
PROM_STACK_RELEASE="${PROM_STACK_RELEASE:-kube-prom-stack}"
...
if ! terraform workspace list | grep -q "${WORKSPACE}"; then
  echo "Terraform workspace ${WORKSPACE} does not exist. Run terraform workspace new/select before deploying." >&2
  exit 1
fi
terraform workspace select "$WORKSPACE" >/dev/null

if ! terraform output >/dev/null 2>&1; then
  echo "Terraform state is not available. Run terraform init/apply first." >&2
  exit 1
fi
...
if [[ -z "$CLUSTER_NAME" ]]; then
  echo "Cluster name output is empty. Ensure EKS module applied for workspace ${WORKSPACE}." >&2
  exit 1
fi

echo "Validating EKS cluster ${CLUSTER_NAME} in ${AWS_REGION}..."
if ! aws eks describe-cluster --name "$CLUSTER_NAME" --region "$AWS_REGION" >/dev/null 2>&1; then
  echo "Cluster ${CLUSTER_NAME} not found. Apply Terraform stack first." >&2
  exit 1
fi
...
echo "Deploying kube-prometheus-stack (${PROM_STACK_RELEASE})..."
...
echo "Deploying Loki stack (${LOKI_RELEASE})..."
